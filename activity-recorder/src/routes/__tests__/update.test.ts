...
describe('Update activity route handler', () => {
    let id: string;
    let cookie: string[];
    let body = { exercise: 'run', duration: 10, distance: 10, description:'test exercise', difficulty:5, photos:null, privacy:'private' };

    beforeEach(() => {
        id = mongoose.Types.ObjectId().toHexString();
        cookie = global.signin();
        body = { exercise: 'run', duration: 10, distance: 10, description:'test exercise', difficulty:5, photos:null, privacy:'private' };
    });
    ...
    it('should return 403 if the user does not own the activity', async () => {
        ...
        await request(app)
            ...
            .set('Cookie', cookie)
            .send({ exercise: 'run', duration: 10, distance: 10, description:'test exercise', difficulty:5, photos:null, privacy:'private' })
            .expect(401);
    });
    ...
    it('should return 202 if correct payload', async () => {
        ...
        await request(app)
            ...
            .set('Cookie', cookie)
            .send({ exercise, duration: 5, distance: 10, description:'test exercise', difficulty:5, photos:null, privacy:'private' })
            .expect(202);
