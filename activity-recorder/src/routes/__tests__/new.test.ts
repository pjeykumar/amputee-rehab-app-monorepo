...
describe('New activity route handler', () => {
    it('has a route handler listening to POST /api/activity', async () => {
        const resp = await request(app).post('/api/activity').send({ exercise: 'run', duration: 10, distance: 10, description:'test exercise', difficulty:5, photos:null, privacy:'private' });
        expect(resp.status).not.toEqual(404);
    });

    it('can only be access if the user is signed in', async () => {
        await request(app).post('/api/activity').send({ exercise: 'run', duration: 10, distance: 10, description:'test exercise', difficulty:5, photos:null, privacy:'private' }).expect(401);
    });
